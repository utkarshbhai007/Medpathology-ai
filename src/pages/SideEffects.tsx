...
const SideEffects = () => {
    ...
    try {
      ...
      if (response && response.aiAnalysis) {
        ...
        const result = {
          ...
          usedFor: ["Medical treatment"],
          mechanism: "Medication mechanism",
          commonDosage: "As prescribed by physician",
          warnings: response.aiAnalysis.contraindications || [],
  ...
  const handlePrediction = async () => {
    ...
    try {
      ...
      if (response && response.aiAnalysis) {
        // Convert PathologyAI response to expected format
        const result = response.aiAnalysis.alerts || [];
  ...
  const handleInteractionCheck = async () => {
    ...
    try {
      ...
      const response = await pathologyAI.checkMedicationSafety(interactionDrugs, {});
